...
import { getNewslineDefaultTags, getUserNewslineTags, getSessionNewslineTags, getNewslinePublications,updateDefaultNewsline } from "../../../../lib/db/newsline"
import { dbLog, dbEnd } from "../../../../lib/db"
import { RedisKey } from 'ioredis';
import { stringify } from 'querystring';
import {Newsline,NewslineDefinition,ExplorerPublication,Publications,NewslineDefinitionItem} from "../../../../lib/types/newsline"
...
export default async function handler(
    ...
    const body = req.body;
    let { sessionid, userslug, newsline, filter, q,update }: { sessionid?: string, userslug?: string, newsline: string, filter: string[], q?: string,update?:number } = body;
    ...
    try {
        ...
        let allPublicationsRaw, allPublications;
        let newslineCategoriesKey = `all-publications-${newsline}`
      ...
      //  for (let i = 0; i < defaultNewsline.length; i++) {
        ...
        const defaultOverlayNewslineDefinition:NewslineDefinition=defaultNewslineDefinition?.map(n=>{
        ...
        for (let i = 0; i < userNewsline.length; i++) {
            let n = userNewsline[i];
            const f = defaultNewslineDefinition.find((f: any) => f.tag == n.tag);
            if (f)
                continue;
            f.default = false;
            defaultOverlayNewslineDefinition.push(f);
        }
        defaultOverlayNewslineDefinition.sort((a:NewslineDefinitionItem,b:NewslineDefinitionItem)=>a.name>b.name?1:a.name<b.name?1:0);
        ...
        for(let i=0;i<allPublications.length;i++){
            ...
            const f=defaultNewsline.find((f:any)=>f.tag==n.tag);
            if(f){
                n.default=f.default;
                n.switch=f.switch;
            }
