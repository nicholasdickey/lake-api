  ...
  try {
    ...
    const text = `Generate ${inastyleof ? `in a style of ${inastyleof}` : ""
     ...
     // } ${occasion == "Birthday" ? `` : ""
      ...
      } ${instructions ? "Additional instructions:'" + instructions + "'." : `Keep it very hallmark like, casual, very terse and acerbic, free of sloganeering, banalities, "let's", etc. Keep it real and casual, like the real people talk.`} Keep it around 400 characters unless instructed otherwise.Output in JSON format {headline,body, tags}.Tags is a comma separated string of tags selected from this set [christmas, thanksgiving,easter,julyfourth,sad,melancholy,cheerful,congratulations,birthday,getwell,love,holiday,winter,fall,summer,spring,anniversary,wedding,mood,travel]. Only tags from this set should be included. The tags should reflect the theme and the mood of the main message and will be used to select appropriate image. Output text as github markdown. ${lighthearted?"Make it hallmark humorous, if possible. Use many emojis.":""} Do not add any meta information, like character count. No hashtags.${language ? "Use language:" + language : ""}`;
