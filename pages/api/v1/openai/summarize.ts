...
export default async function handler(
  ...
  try {
    ...
    try {
      ...
      const messages: ChatCompletionRequestMessage[] = [{ "role": "user", "content": `In style of Raymond Chandler, please summarize in two very short paragraphs or less, keeping only the bare gist, only minimal and essential. Make it brief. Keep it under 280 characters. Split into two paragraphs if needed. Use "pro-Hamas", "Hamas Terror supporters" instead of "pro-Palestinian". Use "Putin's regime" instead of Russia. Use "Illegal Aliens" instead of migrants where the context is in US. Use "Palestinian Arabs" instead of "Palestinians". Categorize using  one appropriate hash tag from this list (#illegals,#politics, #society, #ukraine,#israel,#economy, #foreignaffairs,#military,#culture,#history,#health,#education,#crime,#sports,#science,#outdoors,#religion,#technology,#other). Attach the selected hashtag to the end of the summary. Do not show any other tags. Do not show "other" tag either.:${text}` },];
